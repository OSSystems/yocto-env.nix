...
let
  ...
  fhs = pkgs.buildFHSEnvBubblewrap {
    ...
    targetPkgs = pkgs: (with pkgsForYocto; let
      libxcrypt = if pkgsForYocto ? libxcrypt then pkgsForYocto.libxcrypt else null; # Nixpkgs 20.03
      rpcsvc-proto = if pkgsForYocto ? rpcsvc-proto then pkgsForYocto.rpcsvc-proto else null; # Nixpkgs 20.03
      util-linux = if pkgsForYocto ? util-linux then pkgsForYocto.util-linux else pkgsForYocto.utillinux; # Nixpkgs 20.09
    in
    [
      attr
      bc
